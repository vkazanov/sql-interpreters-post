...
** Лексический и синтаксический анализаторы
   ...
   (scanner_t), который и отдаёт токены один за другим:
   ...
   Синтаксический анализ проводится методом рекурсивного спуска. Сначала создаётся объект parser_t,
...
** Компиляция запросов в промежуточное представление
   ...
   деревьев разбора, то есть структур querycreatetablet и queryinsert. Более сложные запросы SELECT
...
** Операторы
   ...
   Интерфейс у операторов общий и состоит из указателей на функции open/next/close и дополнительной
...
*** Оператор scan
    ...
    кортежи отношения. Внутреннее состояние scan — это указатель на отношение, откуда будут
